...
function createOpenClawEnginePlugin(options = {}) {
  ...
  return {
    apply(ctx) {
      ...
      // workspace/session management services (integrated into this single plugin)
      ...
      async function currentWorkspace() {
        try {
          const root = sandboxPolicySvc && typeof sandboxPolicySvc.workspaceRoot === 'string' ? sandboxPolicySvc.workspaceRoot : undefined
          if (root && wsr) {
        ...
        def(
          ...
          async () => {
            ...
            try {
              ...
              return { ok: true, current, workspaces: all.map(workspaceView) }
            } catch (e) {
              return { ok: false, error: String((e && e.message) || e) }
            }
          },
